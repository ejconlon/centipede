...
class Box[T]:
    ...
    def __iadd__(self, other: Any) -> Box[T]:
        """Defer += operator to the underlying value."""
        self.value = self.value + other
        return self

    def __isub__(self, other: Any) -> Box[T]:
        """Defer -= operator to the underlying value."""
        self.value = self.value - other
        return self

    def __imul__(self, other: Any) -> Box[T]:
        """Defer *= operator to the underlying value."""
        self.value = self.value * other
        return self

    def __itruediv__(self, other: Any) -> Box[T]:
        """Defer /= operator to the underlying value."""
        self.value = self.value / other
        return self

    def __ifloordiv__(self, other: Any) -> Box[T]:
        """Defer //= operator to the underlying value."""
        self.value = self.value // other
        return self

    def __imod__(self, other: Any) -> Box[T]:
        """Defer %= operator to the underlying value."""
        self.value = self.value % other
        return self

    def __ipow__(self, other: Any) -> Box[T]:
        """Defer **= operator to the underlying value."""
        self.value = self.value**other
        return self

    def __ilshift__(self, other: Any) -> Box[T]:
        """Defer <<= operator to the underlying value."""
        self.value = other << self.value
        return self

    def __irshift__(self, other: Any) -> Box[T]:
        """Defer >>= operator to the underlying value."""
        self.value = self.value >> other
        return self

    def __iand__(self, other: Any) -> Box[T]:
        """Defer &= operator to the underlying value."""
        self.value = self.value & other
        return self

    def __ior__(self, other: Any) -> Box[T]:
        """Defer |= operator to the underlying value."""
        self.value = self.value | other
        return self

    def __ixor__(self, other: Any) -> Box[T]:
        """Defer ^= operator to the underlying value."""
        self.value = self.value ^ other
        return self
...
class Flip[T](Comparable["Flip[T]"]):
    ...
    Example:
        >>> from spiny.common import Flip, compare, Ordering
